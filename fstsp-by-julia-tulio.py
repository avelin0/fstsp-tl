...
class CompactFormulation:
    ...
    def __init__(self, inst, big_m=None, murray_rules=False):
        ...
        x = {(i, j, ell): m.add_var(obj=0, var_type=BINARY, name="x({i},{j}__{ell})".format(**locals()))
             for (i, j) in inst.A
             for ell in L.list}

        y = {(i, j, k, ell, ell_): m.add_var(obj=0,
                                                 var_type=BINARY,
        ...
        t = [m.add_var(obj=0,
             ...
             for ell in L.list]
        t[0].ub = 0
        t.append(m.add_var(obj=1, name="t_total".format(**locals())))

        # depot constraints
        m += xsum(x[0, j, 0] for j in inst.V if (0, j) in inst.A) \
                         == xsum(x[j, 0, ell] for j in inst.V if (j, 0) in inst.A \
                                 for ell in L.list[1:]), \
                         "depot"
        ...
        for ell in L.list:
            m.add_constr(xsum(y[i, k, j, l, l_]
                                  for (i, k, j) in inst.D
        ...
        for i in inst.V:
            for ell in L.list:
                m.add_constr(xsum(y[i, k, j, ell, l_]
                                      for k in inst.V_
                                      for j in inst.V if (i, k, j) in inst.D
        ...
        for j in inst.V:
            for ell_ in L.list[1:]:
                m.add_constr(xsum(y[i, k, j, l, ell_]
                                      for i in inst.V
        ...
        for idx, ell in enumerate(L.list[1:]):
            if murray_rules:  # and idx == 0:
                continue
            for ell_ in L.list_ell_prime(ell):
                m.add_constr(t[ell_] - t[ell] <= inst.E
