...
class MurrayFormulation:
    def __init__(self, inst, big_m=5000):
        ...
        x = {(i, j): model.add_var(obj=0, var_type=BINARY, name='x({i},{j})'.format(**locals())) for (i, j) in inst.A}
        ...
        # model.objective = t[inst.V[-1]]

        for j in inst.V_:
            model.add_constr((quicksum(x[i, j]
        ...
        for (i, j) in inst.A:
            if i != 0:
                model.add_constr((u[i] - u[j] + 1 <= (len(inst.V_) + 2) * (1 - x[i, j])
                                  ), "c5")
        ...
        for i in inst.V[:-1]:
            model.add_constr((quicksum(y[i, j, k]
                                       for j in inst.V_
                                       for k in inst.V[1:]
        ...
        for j in inst.V_:
            for k in inst.V[1:]:
                if (0, j, k) in inst.D:
                    model.add_constr((y[0, j, k] <= quicksum(x[h, k]
        ...
        for h in inst.V[:-1]:
            for k in inst.V[1:]:
                if k != h:
                    model.add_constr((t[k] >= t[h] + inst.tau_truck[h][k]
        ...
        for j in inst.V_drone:
            for i in inst.V[:-1]:
                if i != j:
                    model.add_constr((t_[j] >= t_[i] + inst.tau_drone[i][j]
        ...
        for j in inst.V_drone:
            for k in inst.V[1:]:
                if j != k:
                    model.add_constr((t_[k] >= t_[j] + inst.tau_drone[j][k] + inst.sr
                                      - big_m * (1 - quicksum(y[i, j, k]
                                                              for i in inst.V[:-1] if (i, j, k) in inst.D))
                                      ), "c18")

        for (i, j, k) in inst.D:
            model.add_constr((t_[k] - (t_[j] - inst.tau_drone[i][j])
        ...
        for i in inst.V_:
            for j in inst.V_:
                if j != i:
                    model.add_constr((u[i] - u[j] >= 1 - (len(inst.V_) + 2) * p[i, j]
                                      ), "c20")

        for i in inst.V_:
            for j in inst.V_:
                if j != i:
                    model.add_constr((u[i] - u[j] <= -1 + (len(inst.V_) + 2) * (1 - p[i, j])
                                      ), "c21")
        for i in inst.V_:
            for j in inst.V_:
                if j != i:
                    model.add_constr((p[i, j] + p[j, i] == 1
                                      ), "c22")
    ...
    def optimize(self, timelimit, sol=None):
        ...
        try:
            ...
            print('error in write solution')


if __name__ == "__main__":
    arg = Arguments(sys.argv, prefix="murray_")

    folder = r'C:\Users\avelino\OneDrive\programacao\fstsp-mip-eu\data\run'

    instances = [i for i in os.listdir(folder)]
    for instance in instances:
        # if not arg.E:
        #     inst = Instance(arg.instance)
        # else:
        #     inst = Instance(arg.instance, E=arg.E)
        instance = os.path.join(folder,instance)
        print('\n[+]', instance)
        ...
        f = MurrayFormulation(inst)
        f.optimize(arg.timelimit, sol=arg.sol)
        f.solution.print_solution()
        f.solution.write(arg.out)
        #
        # tt = inst.tau_truck
        # td = inst.tau_drone
        ...
        # for (i,j,k) in inst.D:
        #     if f.y[i,j,k].x == 1.0:
        #         print((i,j,k),td[i][j], td[j][k],inst.sl,inst.sr)
                # sm += td[i][j] + td[j][k] + inst.sl + inst.sr
        # print(xsum(inst.tau_truck[i][j] * f.x[i,j] for (i,j) in inst.A)
        #                    + inst.sr         * xsum(f.y[i,j,k] for (i,j,k) in inst.D if i == 0)
        #                    + (inst.sr + inst.sl)    * xsum(f.y[i,j,k] for (i,j,k) in inst.D if i != 0))
        # print(sm)
